...
共有7个 API, 主要使用的2个: registerModel、useModel, 下面介绍这些 API:
...
- effects
  ...
  - 函数内部可以用 this 访问到当前 model 下的 reducers 和 effects 中的方法, 如: this.increment()
...
注: 通常使用这个方法, 不能重复注册同一个 命名空间 的 model, 即 namespace 相同, 如果重复, 控制台会给出报错提示, 不会覆盖之前的 model; 如果你需要动态注入 model, 根据组件挂载注册 model, 可以使用 useDynamicInject 或 dynamicInject, 详细见下方介绍
...
- Component, 为包裹组件, 其 props 将会接收到 namespace 对应的 model 数据, 包装在其 命名空间 下, 如下代码示例:
